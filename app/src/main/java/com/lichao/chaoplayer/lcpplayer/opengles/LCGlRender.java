...
import com.lichao.chaoplayer.lcpplayer.util.LCLog;
...
/**
 * Created by ChaoLi on 2018/5/19 0019 - 13:38
 * Email: lichao3140@gmail.com
 * Version: v1.0
 */
public class LCGlRender implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener {
    ...
    private LCOnGlSurfaceViewOncreateListener lcOnGlSurfaceViewOncreateListener;
    private LCOnRenderRefreshListener lcOnRenderRefreshListener;

    public LCGlRender(Context context) {
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        LCLog.d("onSurfaceChanged, width:" + width + ",height :" + height);
        sWidth = width;
        sHeight = height;
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if(cutimg)
        {
            ...
            if(lcOnGlSurfaceViewOncreateListener != null)
            {
                lcOnGlSurfaceViewOncreateListener.onCutVideoImg(bitmap);
            }
        }
    ...
    public void onFrameAvailable(SurfaceTexture surfaceTexture) {
        LCLog.d("updateSurface");
        if(lcOnRenderRefreshListener != null)
        {
            lcOnRenderRefreshListener.onRefresh();
        }
    }

    public void setlcOnGlSurfaceViewOncreateListener(LCOnGlSurfaceViewOncreateListener lcOnGlSurfaceViewOncreateListener) {
        this.lcOnGlSurfaceViewOncreateListener = lcOnGlSurfaceViewOncreateListener;
    }

    public void setlcOnRenderRefreshListener(LCOnRenderRefreshListener lcOnRenderRefreshListener) {
        this.lcOnRenderRefreshListener = lcOnRenderRefreshListener;
    }
    ...
    {
        String vertexShader = LCShaderUtils.readRawTextFile(context, R.raw.vertex_base);
        String fragmentShader = LCShaderUtils.readRawTextFile(context, R.raw.fragment_mediacodec);
        programId_mediacodec = LCShaderUtils.createProgram(vertexShader, fragmentShader);
        ...
        LCShaderUtils.checkGlError("glBindTexture mTextureID");
        ...
        if(lcOnGlSurfaceViewOncreateListener != null)
        {
            lcOnGlSurfaceViewOncreateListener.onGlSurfaceViewOncreate(surface);
        }
    }
    ...
    {
        String vertexShader = LCShaderUtils.readRawTextFile(context, R.raw.vertex_base);
        String fragmentShader = LCShaderUtils.readRawTextFile(context, R.raw.fragment_yuv);
        programId_yuv = LCShaderUtils.createProgram(vertexShader, fragmentShader);
    ...
    {
        String vertexShader = LCShaderUtils.readRawTextFile(context, R.raw.vertex_base);
        String fragmentShader = LCShaderUtils.readRawTextFile(context, R.raw.fragment_no);
        programId_stop = LCShaderUtils.createProgram(vertexShader, fragmentShader);
